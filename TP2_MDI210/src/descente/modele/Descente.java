...
public class Descente extends Observable implements Runnable  {
	...
	private void directionASuivreSiInterieur() {
		Couple grad_f = pb.gradientf(P);
		if(grad_f.norme()<seuil) {
			finie = true;
		}
		else {
			direction = grad_f.mult(-1);
		}
	...
	private void directionASuivreSiBord(Contrainte c) {
		...
		if(grad_f.norme()<seuil) {
			...
			double prodscal = grad_f.produitScalaire(grad_c);
			if(prodscal <0) {
				Couple u = c.getVecteurUnitaireBord();
				double prodscal2 = grad_f.produitScalaire(u);
				if(estNul(prodscal2)) {
					finie = true;
				}
				if(prodscal2<0) {
	...
	private void directionASuivreSiCoin(Contrainte[] coin) {
		...
		if(grad_f.norme()<seuil) {
			...
			Couple decompo_f1 = Couple.decompose(grad_f, u1, u2);
			if(decompo_f1.x<0 && decompo_f1.y<0) {
				direction = grad_f.mult(-1);
			}
			else {
				Couple decompo_f2 = Couple.decompose(grad_f, n1, n2);
				if(decompo_f2.x>0 && decompo_f2.y>0) {
					finie = true;
				}
				else {
					double produ1 = grad_f.produitScalaire(u1);
					double produ2 = grad_f.produitScalaire(u2);
					if(produ1<produ2) {
						direction = u1;
					}
					else {
	...
	public double chercheSecondPoint(Couple pointDepart, Couple dir) {
		...
		while(t<Double.MAX_VALUE/2) {
			double phiprime_t = pb.phiDerivee(pointDepart, dir, t);
			if(phiprime_t>0) {
				break;
			}
			t = t*2;
	...
	public Couple dichotomie(Couple pointDepart, Couple dir, double t1) {		
		...
		double t = 0;
		double phi_prime = -1;
		while(!estNul(phi_prime)) {
			t = (a+b)/2;
			phi_prime = pb.phiDerivee(pointDepart, dir, t);
			if(phi_prime<=0) {
				a = t;
			}
			else {
