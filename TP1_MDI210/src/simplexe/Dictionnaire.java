...
public class Dictionnaire extends Observable {
	...
	public boolean estRealisable() {
		int i = 1;
		while((i<nbVarBase+1)&&(D[i][0] >= 0)) {
			i = i+1;
		}
		return (i == nbVarBase+1);
	}
	...
	public int chercherPremierIndiceVariableEntrante() {
		int j = 1;
		while((j<nbVarHorsBase+1)&&(D[0][j]<=0)) {
			j = j+1;
		}
		if(j == nbVarHorsBase+1) {
			return 0;
		}
		else {
			return j;
	...
	public int chercherIndiceVariableSortante(int jE)  {
		double min_croiss = Double.MAX_VALUE;
		int indice_min_croiss;
		for(int i=1;i<nbVarBase+1;i++) {
			if(D[i][jE]<0) {
				if(-D[i][0]/D[i][jE]<min_croiss) {
					min_croiss = -D[i][0]/D[i][jE];
				}
			}
		}
		int i =1;
		while((i<nbVarBase+1)&&(-D[i][0]/D[i][jE]!=min_croiss)){
			i = i+1;
		}
		if(i == nbVarBase+1) {
			indice_min_croiss = 0;
	...
	public void pivoter(int iS, int jE) {
		double coeff_x_entrant = D[iS][jE];
		D[iS][jE] = 1/coeff_x_entrant;
		for(int j = 0;j<nbVarHorsBase+1;j++) {
			if(j!=jE) {
				D[iS][j] = -D[iS][j]/coeff_x_entrant;
			}
		}
		double val_x = D[iS][0];
		
		D[0][0] = D[0][0]+val_x*D[0][jE];
		for(int j = 1;j<nbVarHorsBase+1;j++) {
			if(j!=jE) {
				D[0][j] = D[0][j]+D[0][jE]*D[iS][j];
			}
		}
		D[0][jE] = D[0][jE]*D[iS][jE];
		
		for(int i=1;i<nbVarBase+1;i++) {
			if((i!=iS)) {
				D[i][0] = D[i][0]+val_x*D[i][jE];
				for(int j =1;j<nbVarHorsBase+1;j++) {
					if(j!=jE) {
						D[i][j] = D[i][j]+D[i][jE]*D[iS][j];
					}
				}
	...
	public int chercherIndiceVariableEntrantePlusGrandCoeff() {	
		...
		int j =1;
		while((j<nbVarHorsBase+1)&&(D[0][j]!=max_coeff)){
			j = j+1;
		}
		if(j == nbVarHorsBase+1) {
			indice_max_coeff = 0;
	...
	public int chercherIndiceVariableEntranteAvantageuse() {	
		...
		if(non_borne) {
			...
			int j =1;
			int iS = chercherIndiceVariableSortante(j);
			while((j<nbVarHorsBase+1)&&(-D[0][j]*D[iS][0]/D[iS][j]!=max_increase)){
				j = j+1;
				if(j<nbVarHorsBase+1) {
					iS = chercherIndiceVariableSortante(j);
				}
			}
			if(j == nbVarHorsBase+1) {
				indice_max_increase = 0;
			}
			else {
				indice_max_increase = j;
			}
			return indice_max_increase;
		}
	}
	...
	public int chercherIndiceVariableEntrantePlusPetitNumero(){	
		...
		int indice_num_min = 0;
		for(int j=1;j<nbVarHorsBase+1;j++) {
			if(D[0][j]>0) {
				if(tabVarHorsBase[j]<var_min) {
					var_min = tabVarHorsBase[j];
					indice_num_min = j;
				}
			}
		}
		return indice_num_min;
	}
	...
	public int chercherIndiceVariableSortantePlusPetitNumero(int jE) {		
		...
		for(int i=1;i<nbVarBase+1;i++) {
			if(-D[i][0]/D[i][jE]==min_croiss) {
				if(tabVarBase[i]<var_min) {
					var_min = tabVarBase[i];
					indice_num_min =i;
				}
			}
		}
